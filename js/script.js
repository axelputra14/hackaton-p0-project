...
function addingToCart(id) {
  if (data[id - 1].stock == 0) {
    ...
    let checkCheckoutContainer =
      document.getElementsByClassName("checkout-container");
...
function funcRemoveAll() {
  ...
  for (let m = 0; m < toRemoveContainer.length; m++) {
    // console.log(toRemoveContainer[m]);
  ...
  // MASIH ADA SISA JADI ULANG LAGI FOR NYA OWO
  if (toRemoveContainer.length !== 0) {
    for (let m = 0; m < toRemoveContainer.length; m++) {
      let gameId = toRemoveContainer[m].getAttribute("game-id");
